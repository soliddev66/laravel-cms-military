...
@section('content')
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-md-flex justify-content-between align-items-center mb-20">
                        ...
                        <div>
                            <button type="button" class="btn btn-primary mb-3 mr-2" data-toggle="modal" data-target="#counterSectionModal">{{ __('content.section_title_and_desc') }}</button>
    ...
    <div class="modal fade" id="counterSectionModal" tabindex="-1" role="dialog" aria-labelledby="counterSectionModalLabel" aria-modal="false">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0 font-16" id="counterModalLabel">{{ __('content.section_title_and_desc') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                </div>
                <div class="modal-body">
                    @if (isset($counter_section))
                        <form action="{{ route('counter-section.update', $counter_section->id) }}" method="POST" enctype="multipart/form-data">
                            @method('PUT')
                            @csrf
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="title" class="form-control" id="title" value="{{ $counter_section->title }}">
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="desc">{{ __('content.desc') }}</label>
                                        <textarea name="desc" class="form-control" id="desc">{{ $counter_section->desc }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="counter_image">{{ __('content.image') }} ({{ __('content.size') }} 1920 x 1080) (.svg, .jpg, .jpeg, .png)</label>
                                        <input type="file" name="counter_image" class="form-control-file" id="counter_image">
                                        <small id="counter_image" class="form-text text-muted">{{ __('content.recommended_size') }}</small>
                                    </div>
                                        ...
                                        <div class="card">
                                            <div class="card-body">
                                                <div class="avatar-area text-center">
                                                    <div class="media">
                                                        @if (!empty($counter_section->counter_image))
                                                            <a class="d-block mx-auto" href="#" data-toggle="tooltip" data-placement="top" data-original-title="{{ __('content.current_image') }}">
                                                                <img src="{{ asset('uploads/img/general/'.$counter_section->counter_image) }}" alt="counter image" class="rounded w-25">
                                                            </a>
                                                        @else
                                                            <a  class="d-block mx-auto" href="#" data-toggle="tooltip" data-placement="top" data-original-title="{{ __('content.current_image') }}">
