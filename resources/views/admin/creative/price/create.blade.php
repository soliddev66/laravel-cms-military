...
@section('content')
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-md-flex justify-content-between align-items-center mb-20">
                        ...
                        <div>
                            <button type="button" class="btn btn-primary  mb-3 mr-2" data-toggle="modal" data-target="#sectionModal">{{ __('content.section_title') }}</button>
                    ...
                    @if (count($prices) > 0)
                        <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
                            ...
                            @foreach ($prices as $price)
                                <tr>
                                    <td>{{ $i++ }}</td>
                                    <td><i class="{{ $price->icon }}"></i> {{ $price->icon }}</td>
                                    <td>{{ $price->title }}</td>
                                ...
                                <!-- Modal -->
                                    <div class="modal fade" id="deleteModel{{ $price->id }}" tabindex="-1" role="dialog">
    ...
    <div class="modal fade" id="sectionModal" tabindex="-1" role="dialog" aria-labelledby="sectionModalLabel" aria-modal="false">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0 font-16" id="serviceModalLabel">{{ __('content.section_title') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                </div>
                <div class="modal-body">
                    @if (isset($price_section))
                        <form action="{{ route('price-section.update', $price_section->id) }}" method="POST">
                            @method('PUT')
                            @csrf
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="title" class="form-control" id="title" value="{{ $price_section->title }}">
                                    </div>
                                </div>
                            </div>
                        ...
                        @else
                        <form action="{{ route('price-section.store') }}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select name="time" class="form-control" id="time">
                                        ...
                                        <option value="0">{{ __('content.yearly') }}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-12">
